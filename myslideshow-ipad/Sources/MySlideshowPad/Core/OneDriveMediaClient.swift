...
///
/// myslideshow(Mac版)の`Core/OneDriveMediaClient.swift`から無変更で移植したもの ―
/// `URLSession`(Foundationのみ)で完結しており、iOS上でも無変更で動く。認証フロー
/// (匿名Badgerトークン発行 → 共有リンクの解決[+redeem] → children一覧の再帰取得)・
/// 非公開の内部APIである点・署名付きダウンロードURLが1時間程度で失効する点は
/// すべてMac版と同じ(mytubeの`Core/OneDriveShareClient.swift`が大元)。
enum OneDriveMediaClient {
    ...
    /// フォルダ構造を全階層たどる(深さ制限なし)。**呼び出し側は直接これを呼ばず、
    /// `scanWithRetry(shareURL:)`経由で呼ぶこと**(下記参照)。
    ...
    /// (`ConcurrencyLimiter`で同時実行数を絞る、Mac版と同じ設計)。
