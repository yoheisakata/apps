...
{ id:"d2-37", dom:2, cat:"セキュリティ/コンプライアンス",
  q:"アプリケーション層への SQL インジェクションや不正な Bot からの保護と、大規模な DDoS 攻撃時の課金保護・専門チーム支援が必要です。適した組み合わせは?",
  choices:[
    "AWS WAF（マネージドルール＋レート制限）と AWS Shield Advanced",
    "セキュリティグループとネットワーク ACL",
    "GuardDuty と Inspector",
    "Network Firewall と Macie"],
  answer:0,
  note:"WAF は L7 の検査（SQLi/XSS のマネージドルール、Bot Control、レートベースルール）。Shield Standard は無償で L3/L4 を自動保護し、Shield Advanced は高度な検知・DDoS 対応チーム（SRT）・スケーリング費用の払い戻しを提供する。\nGuardDuty は脅威検知、Inspector は脆弱性スキャン、Macie は S3 の機密データ検出で、いずれも防御そのものではない。" },
...
{ id:"d4-28", dom:4, cat:"可用性/DR",
  q:"オンプレの DR サイトを廃止し、AWS をリカバリ先にします。既存の仮想マシンをそのまま復旧先として使いたく、DR 用の常時稼働コストは最小にしたい。適した構成は?",
  choices:[
    "AWS Elastic Disaster Recovery で低コストのステージング領域にレプリケートし、災害時のみ本番サイズの EC2 を起動する",
    "本番と同一構成の EC2 を AWS 側に常時起動しておく",
    "毎日のバックアップを S3 Glacier に保存する",
    "AWS Backup でオンプレの VM を直接バックアップする"],
  answer:0,
  note:"DRS のステージング領域は小さなインスタンスと安価なストレージで構成されるため、パイロットライトに近いコストで RPO 秒単位・RTO 分単位を実現できる。訓練起動も本番に影響なく行える。\n常時同一構成はマルチサイト相当でコストが高すぎ、Glacier バックアップでは RTO を満たせない。" },
...
{ id:"md-01", dom:1, cat:"セキュリティ/コンプライアンス",
  q:"ある企業は AWS Organizations で数十のアカウントを管理している。セキュリティ部門は、開発用 OU 配下のすべてのアカウントで、承認済みリージョン（ap-northeast-1 と us-east-1）以外でのリソース作成を強制的に禁止したい。個々のアカウント管理者が回避できないようにする必要がある。最も適切な方法はどれか。",
  choices:[
    "各アカウントの IAM ユーザーに、リージョンを制限する IAM 許可ポリシーを付与する",
    "開発用 OU に対して、aws:RequestedRegion を条件に非承認リージョンを Deny する SCP を適用する",
    "各アカウントで AWS Config ルールを作成し、非承認リージョンのリソースを自動削除する",
    "IAM Identity Center のアクセス許可セットでリージョン制限を設定する"],
  answer:1, fig:"scp",
  note:"SCP は OU 配下の全アカウント（root ユーザー含む）の最大許可範囲を絞り込むガードレールであり、aws:RequestedRegion 条件で非承認リージョンを Deny すれば個々の管理者が回避できない。IAM の許可（選択肢1・4）は管理者が変更・回避できる。Config ルールは事後検知/削除で予防にならず、グローバルサービス操作の妨げにもなりうる。\n参照: AWS Organizations SCP。" },

{ id:"md-02", dom:1, cat:"IAM/認証・認可",
  q:"本番アカウント A のログ用 S3 バケットに、監査アカウント B の分析ツールから読み取りアクセスさせたい。長期的な認証情報の配布を避け、最小権限で実現する構成はどれか。",
  choices:[
    "アカウント A で IAM ユーザーを作成し、そのアクセスキーをアカウント B に共有する",
    "アカウント A にクロスアカウント IAM ロールを作成し信頼ポリシーでアカウント B を許可、アカウント B のプリンシパルが sts:AssumeRole で一時認証情報を取得する",
    "S3 バケットポリシーでアカウント B の全 IAM ユーザーを Principal: * で許可する",
    "アカウント B のリソースをアカウント A に移設して同一アカウント内でアクセスする"],
  answer:1, fig:"assumerole",
  note:"クロスアカウントアクセスのベストプラクティスは、リソース側アカウントにロールを作り信頼ポリシーで相手アカウントを許可し、sts:AssumeRole による短命の一時認証情報を使うこと。アクセスキーの配布はローテーション・漏洩リスクが高い。Principal: * は過剰許可で危険。リソースの移設は不要。\n参照: AWS STS、クロスアカウント IAM ロール。" },

{ id:"md-03", dom:1, cat:"アカウント/組織管理",
  q:"企業は数百のアカウントを標準化されたガードレール、集約ログ、SSO 付きで迅速に発行できる基盤を最小の運用負荷で構築したい。最も適切なサービスはどれか。",
  choices:[
    "手動で各アカウントを作成し、CloudFormation StackSets でベースラインを都度適用する",
    "AWS Control Tower を使い、Account Factory でアカウントをプロビジョニングし、予防的/発見的ガードレールと集約ログを自動適用する",
    "AWS Config のみを全アカウントに個別設定する",
    "各チームに個別のスタンドアロンアカウントを作らせ、後から統合する"],
  answer:1,
  note:"Control Tower は Organizations、SCP ベースの予防的ガードレール、Config ベースの発見的ガードレール、集約ログ用の Log Archive アカウント、IAM Identity Center による SSO を含むランディングゾーンを自動構築し、Account Factory で標準化アカウントを発行できる。手動作成は運用負荷が高く、Config だけ／後から統合では基盤要件を満たさない。\n参照: AWS Control Tower、Account Factory。" },

{ id:"md-04", dom:1, cat:"ネットワーク",
  q:"50 個の VPC が相互に通信する必要があり、現在はフルメッシュの VPC ピアリングで管理が破綻している。オンプレミスとの接続も 1 か所に集約したい。最も拡張性が高い構成はどれか。",
  choices:[
    "すべての VPC 間で VPC ピアリングを追加し続ける",
    "各 VPC を AWS Transit Gateway にアタッチし、ルートテーブルで通信を制御、Direct Connect / VPN も Transit Gateway に集約する",
    "すべての VPC を 1 つの巨大な VPC に統合する",
    "各 VPC に NAT Gateway を置いてインターネット経由で通信させる"],
  answer:1, fig:"tgw",
  note:"Transit Gateway はハブアンドスポーク型で VPC 間およびオンプレミス接続を集約でき、ピアリングのフルメッシュ（N×(N-1)/2 の管理）を解消する。ルートテーブルでセグメント分離も可能。ピアリング追加は破綻の継続、巨大 VPC への統合は非現実的で影響範囲が大きい、内部通信をインターネット経由にするのは論外。\n参照: AWS Transit Gateway。" },
...
{ id:"md-06", dom:1, cat:"セキュリティ/コンプライアンス",
  q:"企業は Organizations 内すべてのアカウントの API アクティビティを、改ざんされにくい形で 1 つの中央アカウントに集約したい。新規アカウントも自動的に対象にしたい。最適な方法はどれか。",
  choices:[
    "各アカウントで個別に CloudTrail 証跡を作成し、手動で集約する",
    "管理アカウントから組織証跡 (organization trail) を作成し、中央のログアーカイブアカウントの S3 バケットに配信する",
    "CloudWatch Logs を各アカウントで有効にし、メールで転送する",
    "VPC フローログのみを集約する"],
  answer:1,
  note:"CloudTrail の組織証跡は、管理アカウント（または委任管理者）が作成し、既存および将来のメンバーアカウントすべてのイベントを自動的に集約対象とし、中央 S3 バケットへ配信できる。バケットポリシーとオブジェクトロックで改ざん耐性も高められる。個別証跡は運用負荷と抜け漏れ、メール転送は非現実的、フローログは API アクティビティを網羅しない。\n参照: AWS CloudTrail organization trail。" },

{ id:"md-07", dom:1, cat:"セキュリティ/コンプライアンス",
  q:"セキュリティチームは、Organizations の管理アカウントに高い権限を集中させることを避けつつ、GuardDuty や Security Hub を専用のセキュリティアカウントから組織全体で運用したい。適切な手法はどれか。",
  choices:[
    "管理アカウントの root ユーザーの認証情報をセキュリティチームに共有する",
    "セキュリティアカウントを各サービスの委任管理者 (delegated administrator) に指定し、そのアカウントから組織全体の設定を管理する",
    "すべてのメンバーアカウントに Admin IAM ユーザーを作成する",
    "管理アカウントでのみ各セキュリティサービスを個別運用する"],
  answer:1,
  note:"多くの AWS セキュリティ/ガバナンスサービス（GuardDuty、Security Hub、Config、Macie 等）は委任管理者をサポートし、管理アカウントに権限を集中させずに専用アカウントから組織全体を集中管理できる。これは管理アカウントの使用を最小化するベストプラクティスに沿う。root 認証情報の共有や全アカウントへの Admin 作成は重大なセキュリティ違反、管理アカウントでの個別運用は権限集中を招く。\n参照: AWS Organizations delegated administrator。" },

{ id:"md-08", dom:1, cat:"ネットワーク",
  q:"ある SaaS プロバイダーは、自社 VPC 内のサービスを、多数の顧客 VPC に対して、IP 重複を気にせず、インターネットや VPC ピアリングを使わずに一方向で公開したい。最適な構成はどれか。",
  choices:[
    "各顧客と VPC ピアリングを結ぶ",
    "サービスの前段に Network Load Balancer を置き、VPC エンドポイントサービス (AWS PrivateLink) を作成、顧客はインターフェイス VPC エンドポイントで接続する",
    "サービスをパブリックサブネットに置き、インターネット経由で公開する",
    "Transit Gateway で全顧客 VPC を相互接続する"],
  answer:1,
  note:"PrivateLink はエンドポイントサービス（NLB 背後）とインターフェイスエンドポイントにより、一方向・非公開・CIDR 重複に依存しないサービス公開を実現する。ピアリングと TGW は双方向接続で CIDR 重複に弱くマルチテナントに不向き、インターネット公開は要件に反する。\n参照: AWS PrivateLink、VPC エンドポイントサービス。" },
...
{ id:"md-11", dom:4, cat:"移行",
  q:"オンプレミスの Oracle データベースを Amazon Aurora PostgreSQL へ、ダウンタイムを最小化して移行したい。スキーマとストアドプロシージャの変換も必要である。適切なツールの組み合わせはどれか。",
  choices:[
    "DataSync でファイルとしてコピーする",
    "AWS Schema Conversion Tool (SCT) でスキーマ/コードを変換し、AWS DMS の継続的レプリケーション (CDC) で最小ダウンタイム移行を行う",
    "スナップショットを取得して Aurora に直接復元する",
    "mysqldump でエクスポートしインポートする"],
  answer:1,
  note:"異種エンジン間（Oracle→Aurora PostgreSQL）ではスキーマ差異があるため SCT でスキーマ/PL/SQL を変換し、DMS の CDC（変更データキャプチャ）で継続レプリケーションを行うことで最小ダウンタイムのカットオーバーが可能。DataSync は DB 移行に不適、スナップショット復元はエンジンが異なり不可、mysqldump は Oracle と無関係で異種変換もできない。\n参照: AWS DMS、AWS SCT。" },

{ id:"md-12", dom:4, cat:"移行",
  q:"数百台のオンプレミスサーバーの依存関係や使用状況を移行前に把握し、移行の進捗を単一ビューで追跡したい。適切なサービスの組み合わせはどれか。",
  choices:[
    "AWS Config と CloudWatch",
    "AWS Application Discovery Service で構成・依存関係・使用状況を収集し、AWS Migration Hub で発見データと移行進捗を一元的に追跡する",
    "AWS Trusted Advisor で移行計画を立てる",
    "Amazon Inspector で依存関係を可視化する"],
  answer:1,
  note:"Application Discovery Service（エージェントベース/エージェントレス）はサーバー構成、パフォーマンス、ネットワーク依存関係を収集し、Migration Hub がそのデータと各移行ツールの進捗を集約して単一ビューで可視化する。Config/Trusted Advisor/Inspector はそれぞれ構成監査・推奨・脆弱性検査が目的で、移行の発見/追跡には設計されていない。\n参照: AWS Application Discovery Service、AWS Migration Hub。" },

{ id:"md-13", dom:4, cat:"移行",
  q:"オンプレミス NAS 上の 200 TB のファイルを、既存のネットワーク回線を使って Amazon S3 と Amazon EFS へ、増分同期・整合性検証付きで定期転送したい。最適なサービスはどれか。",
  choices:[
    "AWS Snowball Edge を注文して郵送する",
    "AWS DataSync のエージェントをオンプレに配置し、S3/EFS への高速・増分・検証付き転送をスケジュールする",
    "S3 コンソールから手動でドラッグ＆ドロップする",
    "Storage Gateway のテープゲートウェイを使う"],
  answer:1,
  note:"DataSync はオンプレミス（NFS/SMB）から S3/EFS/FSx へ、ネットワーク回線経由で並列高速転送・増分同期・データ整合性検証・スケジュール実行を提供し、継続的な同期に適する。Snowball は一括物理搬送向けで定期増分同期には非効率、手動アップロードは非現実的、テープゲートウェイはバックアップ/仮想テープ用途。\n参照: AWS DataSync。" },

{ id:"md-14", dom:4, cat:"移行",
  q:"ネットワーク帯域が乏しい遠隔拠点から 500 TB のデータを AWS に移行する必要がある。回線での転送では数か月かかると試算された。最も現実的な方法はどれか。",
  choices:[
    "AWS DataSync で回線経由転送する",
    "Direct Connect を新設して転送する",
    "AWS Snowball Edge デバイスを取り寄せてローカルにデータをコピーし、物理的に AWS へ搬送してインポートする",
    "VPN を追加して並列で転送する"],
  answer:2,
  note:"帯域が乏しく回線転送に数か月かかる大容量オフライン移行は Snow ファミリー（Snowball Edge 等）の物理搬送が適し、数日〜数週間で S3 に取り込める。DataSync・Direct Connect 新設・VPN 追加はいずれも限られた回線帯域に依存し時間・コストの制約を解消できない。\n参照: AWS Snowball Edge、Snow Family。" },

{ id:"md-15", dom:4, cat:"モダナイゼーション",
  q:"モノリシックなオンプレ Java アプリを、リスクを抑えながら段階的にマイクロサービス化して AWS 上でコンテナ運用したい。全面書き換えは避けたい。適切なアプローチはどれか。",
  choices:[
    "モノリスを一括で書き直し、完成後に一度で切り替える（ビッグバン移行）",
    "Strangler Fig パターンで機能を少しずつ切り出して Amazon ECS/EKS 上のマイクロサービスに移し、ルーティング層で新旧に振り分けながら段階的に置き換える",
    "モノリスをそのまま 1 つの巨大な Lambda 関数に載せる",
    "モノリスを EC2 にリホストするだけで最新化完了とする"],
  answer:1,
  note:"Strangler Fig パターンは、API Gateway/ALB 等のルーティング層で新旧を振り分けつつ機能単位で段階的にマイクロサービス（ECS/EKS コンテナ）へ移行し、最終的にモノリスを「絞め殺す」ため、リスクを抑えた最新化ができる。ビッグバンは高リスク、巨大 Lambda は実行時間/サイズ制約に反し不適、リホストのみでは最新化にならない。\n参照: Strangler Fig パターン、Amazon ECS/EKS。" },
...
{ id:"md-17", dom:2, cat:"データベース",
  q:"グローバル展開する金融アプリで、主リージョンの Aurora PostgreSQL に対し、別リージョンで読み取りレイテンシーを下げつつ、リージョン障害時に RPO 1 秒未満・RTO 1 分程度で昇格したい。マネージドで最小の運用負荷が求められる。最適な構成はどれか。",
  choices:[
    "Aurora のクロスリージョンリードレプリカをバイナリログレプリケーションで作成する",
    "Aurora Global Database をセカンダリリージョンに構成する",
    "RDS Multi-AZ をセカンダリリージョンにも独立して構築し、アプリで同期する",
    "DynamoDB Global Tables に移行する"],
  answer:1,
  note:"Aurora Global Database は専用インフラによるストレージレベルレプリケーションで、通常 1 秒未満のレプリケーション遅延と高速なマネージドフェイルオーバー（昇格）を提供し、リモートリージョンの読み取りレイテンシーも低減する。従来のクロスリージョンリードレプリカは遅延が大きく昇格も遅い。アプリ側での同期は実装負荷が高く、DynamoDB はリレーショナル要件に合わない。\n参照: Amazon Aurora Global Database。" },

{ id:"md-18", dom:2, cat:"データベース",
  q:"複数リージョンのユーザーが同一データを読み書きするモバイルバックエンドを設計している。各リージョンでローカルな低レイテンシー読み書きを実現し、リージョン間で自動的にデータを双方向レプリケートしたい。運用負荷を最小にする選択はどれか。",
  choices:[
    "単一リージョンの DynamoDB テーブル + DAX",
    "DynamoDB Global Tables（マルチリージョン、マルチアクティブ）",
    "DynamoDB + Lambda で自作のクロスリージョンレプリケーション",
    "DynamoDB Streams を Kinesis 経由で他リージョンへ手動反映"],
  answer:1,
  note:"DynamoDB Global Tables はマルチアクティブなマルチリージョンレプリケーションをフルマネージドで提供し、各リージョンでローカル読み書きを可能にする（Last Writer Wins による競合解決）。自作レプリケーションは運用負荷が高く整合性管理も複雑。DAX は単一リージョンのためマルチリージョン低レイテンシー要件を満たさない。\n参照: DynamoDB Global Tables。" },

{ id:"md-19", dom:2, cat:"モダナイゼーション",
  q:"Lambda が SQS 標準キューからメッセージを処理するが、特定の不正メッセージで繰り返し失敗し、正常メッセージの処理まで滞留している。失敗メッセージを隔離し、正常処理への影響を減らす設計として最適なものはどれか。",
  choices:[
    "SQS キューの可視性タイムアウトを 0 にする",
    "SQS に Dead Letter Queue を設定し maxReceiveCount を超えたメッセージを退避する",
    "Lambda の同時実行数を 1 に固定する",
    "FIFO キューに変更する"],
  answer:1,
  note:"DLQ を設定し maxReceiveCount を超えて処理失敗したメッセージを自動退避すれば、毒メッセージ（poison message）が正常処理をブロックせず、後で個別調査できる。可視性タイムアウト 0 は同一メッセージが即再配信され問題を悪化させる。同時実行数の固定はスループットを下げるだけで根本解決にならない。FIFO は順序保証を加えるが失敗メッセージの滞留は解決しない。\n参照: Amazon SQS Dead Letter Queue。" },
...
{ id:"md-22", dom:2, cat:"モダナイゼーション",
  q:"チームはコンテナ化したステートレス API を最小の運用負荷で稼働させたい。Kubernetes の専門知識はなく、EC2 インスタンスの管理やパッチ適用も避けたい。トラフィックに応じて自動スケールさせたい。最適な選択はどれか。",
  choices:[
    "Amazon EKS + セルフマネージド EC2 ノードグループ",
    "Amazon ECS on AWS Fargate",
    "Amazon EKS + マネージドノードグループ（EC2）",
    "EC2 上に自前で Docker を構築"],
  answer:1,
  note:"ECS on Fargate はサーバーレスなコンテナ実行環境で、EC2 のプロビジョニング・パッチ・スケーリング管理が不要、かつ Kubernetes の学習コストもない。EKS はセルフマネージド/マネージドいずれのノードグループでも EC2（および K8s）の管理が残る。自前 Docker は運用負荷が最大。\n参照: Amazon ECS, AWS Fargate。" },
...
{ id:"md-24", dom:2, cat:"セキュリティ/コンプライアンス",
  ...
  answer:[0,2],
  note:"AWS WAF は SQLi/XSS などの L7 攻撃をルール（マネージドルール含む）でブロックできる。Shield Advanced は高度な L3/L4 DDoS 保護、24x7 の DDoS 対応チームによる支援、DDoS 起因のスケーリング費用補償を提供する。Security Group での都度ブロックは運用が手動で追いつかない。Shield Standard は基本保護のみで費用補償や高度機能がない。NACL はステートレスで L7 攻撃には無力。\n参照: AWS WAF, AWS Shield Advanced。" },
...
{ id:"md-27", dom:3, cat:"ネットワーク",
  q:"グローバルユーザー向けに、TCP/UDP ベースの非 HTTP なリアルタイムゲーム API を提供している。世界中からの接続で低レイテンシーと高速フェイルオーバー、静的 Anycast IP を求めている。最適なサービスはどれか。",
  choices:[
    "Amazon CloudFront",
    "AWS Global Accelerator",
    "Route 53 レイテンシールーティングのみ",
    "リージョンごとに個別 ALB を公開しクライアントで切替"],
  answer:1,
  note:"Global Accelerator は静的 Anycast IP を提供し、AWS のグローバルネットワーク経由で TCP/UDP を含む非 HTTP トラフィックを最寄りエッジから最適経路でルーティングし、高速なリージョンフェイルオーバーを行う。CloudFront は主に HTTP(S) のコンテンツ配信/キャッシュ向けで、任意 TCP/UDP のリアルタイム API には不適。Route 53 は DNS キャッシュにより切替が遅い。クライアント側切替は実装負荷が高い。\n参照: AWS Global Accelerator。" },

{ id:"md-28", dom:3, cat:"モダナイゼーション",
  q:"本番の Lambda ベース API で、新バージョンをリリースする際に一部トラフィックのみで検証し、CloudWatch アラームで問題を検知したら自動ロールバックしたい。最小の運用負荷で実現する方法はどれか。",
  choices:[
    "Lambda のエイリアスと加重ルーティング + CodeDeploy のカナリアデプロイ + CloudWatch アラームで自動ロールバック",
    "新旧を別関数として作り、クライアント側で切り替える",
    "全トラフィックを一度に新バージョンへ切り替える（All-at-once）",
    "Auto Scaling グループのローリング更新を使う"],
  answer:0,
  note:"Lambda エイリアスの加重ルーティングと CodeDeploy のカナリア（例: Canary10Percent5Minutes）を組み合わせると、一部トラフィックで新バージョンを検証し、関連付けた CloudWatch アラームが発報した場合に自動ロールバックできる。クライアント側切替は改修が必要。All-at-once は段階検証がなくリスク大。Auto Scaling のローリング更新は Lambda ではなく EC2 向けの手法。\n参照: AWS CodeDeploy for Lambda, Lambda alias traffic shifting。" },
