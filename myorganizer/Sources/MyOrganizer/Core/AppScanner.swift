...
enum AppScanner {
    static func scanAll() -> [AppInfo] {
        ...
        // 自作アプリは /Applications/MyApplications/ にまとめてインストールしている。
        let myApps = URL(fileURLWithPath: "/Applications/MyApplications")
        if fm.fileExists(atPath: myApps.path) {
            directories.append(myApps)
        }
