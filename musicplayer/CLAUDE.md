...
./build_app.sh       # dist/MusicPlayer.app を生成
...
- **`LinkResolver`**(`LinkResolver.swift`): 曲リンクを再生可能な `Track` に解決する唯一の入口。
  ...
  - YouTube → `yt-dlp -x --audio-format mp3 --print "after_move:%(id)s\t%(title)s\t%(thumbnail)s"`
    を `Process` で実行し、キャッシュ(`~/Library/Application Support/MusicPlayer/cache/%(id)s.mp3`)
...
- **`PlaylistStore`**(`PlaylistStore.swift`): `tracks: [Track]` を保持し、
  `~/Library/Application Support/MusicPlayer/playlist.json` に JSON で永続化する。
  ...
  `~/Library/Application Support/MusicPlayer/import-errors.log` に
