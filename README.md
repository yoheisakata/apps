...
ルートの `index.html` がランチャー（ホーム画面）で、各アプリへのリンクをアイコングリッドにまとめています（「ゲーム」「その他」「学習」の3グループ）。GitHub Pages で公開しています。

---

## 📱 Webアプリ

### 🔢 すうじくえすと — `pwa/suji-quest/`
たしざん・くく（かけ算）を練習できる子ども向け学習アプリ（PWA）。正解すると⭐スターがたまる。
- たしざん：難易度別（レベル1〜5）の3択問題、絵文字による数の視覚化
- くく（かけざん）：段ごとの九九学習カード、音声読み上げ付き
- 旧「まなびアプリ」（`kids-learning-app/`）から2026-08-18に分割。姉妹アプリは下記「もじくえすと」

### 🔤 もじくえすと — `pwa/moji-quest/`
ひらがな・ローマ字タイピングを練習できる子ども向け学習アプリ（PWA）。正解すると⭐スターがたまる。
- ひらがな：お手本を正しい書き順でなぞる練習（KanjiVGの筆順データを使用）
- タイピング：ひらがな1文字／かんたんなことばをローマ字入力で練習
- 旧「まなびアプリ」（`kids-learning-app/`）から2026-08-18に分割。姉妹アプリは上記「すうじくえすと」

### 🌏 ちきゅうをまなぼう — `pwa/earth/`
地球や自然について学べる子ども向け学習アプリ。

### 🚄 新幹線マップ — `pwa/shinkansen/`
日本の新幹線の路線をマップで見られるアプリ。

### 🔮 タロット占い — `pwa/tarot/`
タロットカードによる占いアプリ。カードの意味を学べる **タロットクイズ**（`tarot/quiz.html`）付き。

### 🐘 PostgreSQL 強化書 — `pwa/pgquiz/`
...
### ⚽ ワールドカップ 2026 — `pwa/world-cup-2026/`
