#!/usr/bin/env bash
# build.sh — build the Photo Gallery app (native Swift macOS local-folder photo browser).
#
# Usage:
#   ./build.sh install   build + copy to /Applications (launch from Spotlight/Launchpad)
#   ./build.sh app       build a double-clickable "Photo Gallery.app" in build/
...
# in Finder → Open With → Photo Gallery.
set -euo pipefail

DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BIN="$DIR/build/photo-gallery"
SRC="$DIR/Sources/main.swift"
APP="$DIR/build/Photo Gallery.app"
INSTALLED="/Applications/Photo Gallery.app"
...
build() {
  ...
  swiftc -O -framework AppKit -o "$BIN" "$SRC"
  echo "Built $BIN"
}
...
build_app() {
  needs_build && build
  echo "Packaging Photo Gallery.app…"
  rm -rf "$APP"
  mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
  cp "$BIN" "$APP/Contents/MacOS/photo-gallery"
...
<plist version="1.0">
<dict>
  <key>CFBundleName</key>            <string>Photo Gallery</string>
  <key>CFBundleDisplayName</key>     <string>Photo Gallery</string>
  <key>CFBundleIdentifier</key>      <string>com.yosakata.photo-gallery</string>
  <key>CFBundleVersion</key>         <string>${VERSION}</string>
  <key>CFBundleShortVersionString</key> <string>${VERSION}</string>
  <key>CFBundlePackageType</key>     <string>APPL</string>
  <key>CFBundleExecutable</key>      <string>photo-gallery</string>
...
case "${1:-install}" in
  ...
  install)
    ...
    echo
    echo "Launch from Spotlight (⌘Space → \"Photo Gallery\") or Launchpad."
    echo "Right-click a folder → Open With → Photo Gallery."
    ;;
