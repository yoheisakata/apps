...
- Boxart: fetched from libretro-thumbnails (No-Intro naming) via `ThumbnailLoader` (max 6 concurrent, transient errors retried with backoff). Japan-region art is preferred — `ScannedROM.thumbnailCandidates` tries `(Japan)` / `(Japan, USA)` / `(World)` before the ROM's own region tag. CJK-titled files are resolved through `TitleMap` (`~/Library/Application Support/RetroGames/title-map.json`, keys `"NES|日本語タイトル"` NFC-normalized → romanized No-Intro name, generated from the user's old 7z collection); unmapped CJK titles get an empty candidate list (placeholder card, no remote lookup). Cache lives in `~/Library/Application Support/RetroGames/Thumbnails/` with a `-jp` filename suffix (bump the suffix to invalidate if the preference logic changes).
