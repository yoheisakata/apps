...
enum VaultFile {
    ...
    static var directoryURL: URL {
        ...
        return appSupport.appendingPathComponent("MyPass", isDirectory: true)
    }
    ...
    // MARK: - 暗号化バックアップ (MyPass 専用形式 .passmanbackup)

    /// バックアップファイルのマジック。MyPass 以外のアプリでは開けない独自コンテナにするための識別子。
    ...
    /// PassMan → MyPass の改名(2026-08-12)後も旧名のまま。書き出し済みバックアップの拡張子と
    /// マジックはファイルの中身そのものなので、変えると過去のバックアップが
    /// 復元パネルの `allowedContentTypes` から外れ、`decodeBackup` のマジック照合でも弾かれる。
    static let backupFileExtension = "passmanbackup"

    /// 現在の暗号化エンベロープを MyPass 専用バックアップコンテナにエンコードする。
    ...
    /// マジックが一致しなければ MyPass のバックアップではないので弾く。
