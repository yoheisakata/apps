...
final class VaultModel: ObservableObject {
    ...
    /// 現在の vault を MyPass 専用の暗号化バックアップ(.passmanbackup)としてエンコードする。
    ...
    /// MyPass バックアップから復元する。バックアップ作成時のマスターパスワードで
    ...
    func restore(from data: Data, password: String) -> Bool {
        do {
            ...
            errorMessage = "MyPass のバックアップファイルではありません"
            return false
        } catch {
    ...
    func unlockWithBiometrics() {
        ...
        BiometricStore.retrieveDEK(reason: "MyPass のロックを解除") { [weak self] dekData in
