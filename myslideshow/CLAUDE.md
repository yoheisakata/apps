...
- **`Views/SlideshowView.swift`** — このアプリの中心。写真は`Settings.photoDurationSeconds`
  ...
  - **右下の日付ラベル**(`dateLabel(for:)`、2026-08-29追加) — `current.capturedDate`が
    ...
    妨げないため)。**フォントは`.system(size: 36, weight: .bold)`**(2026-08-31、
    「日付を大きくしてほしい」→(28ptにした後)「日付がまだ小さい」→(48ptにした後)
    「少し大きくなりすぎたので、少しだけ小さく」という3段階の要望で
    `.callout.weight(.medium)`→28pt→48pt→36ptと調整、背景カプセルの`padding`も
    10/5→18/10→24/14→20/12。**この最後の縮小はiPad版にも同時に適用した**
    (下記`myslideshow-ipad/CLAUDE.md`の同期漏れの教訓を踏まえ、今回は両方
    同時に直した)。
...
- **`Views/HomeView.swift`** — `links`を`ForEach`し、それぞれ`linkSection(_:)`が
  ...
  不要になった。
  - **「スライドショー設定」`GroupBox`内の並び順は、上から
    ランダム再生(`Toggle`)→表示モード(`Picker`)→写真の表示時間→時間制限**
    (2026-08-31、「ランダム設定を最初にもってきて」という要望への対応 ―
    当初は表示モードが先頭だった)。
