...
Static single-file apps: `earth`, `tarot` (`index.html` 占い + `quiz.html` クイズ), `shinkansen`, `pgquiz` (PostgreSQL 17以降向けのクイズ/フラッシュカード学習アプリ、104問・13カテゴリ), `shogi` (将棋; 対AI対戦/2人対戦、自前実装の合法手生成・王手/詰み判定・簡易ネガマックスAI — `mygames`の将棋タブとは独立した別実装。既知の簡略化: 打ち歩詰め・千日手は未判定。詳細は `shogi/CLAUDE.md`).
