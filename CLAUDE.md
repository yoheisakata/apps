...
Static single-file apps: `earth`, `tarot` (`index.html` 占い + `quiz.html` クイズ), `shinkansen`, `pgquiz` (PostgreSQL 17以降向けのクイズ/フラッシュカード学習アプリ、104問・13カテゴリ), `rock-collections` (「いし＆こうぶつ図鑑」; カメラで撮影した石・鉱物を Claude API (Anthropic Messages API, ユーザーが自分の API キーを入力し `localStorage` に保存) で判定し、レア度つきの図鑑としてコレクションする子ども向けアプリ。外部 CDN の Font Awesome を読み込んでいる点が他の web アプリと異なる。manifest.json + sw.js でインストール可能な PWA — Kindle Fire 等のホーム画面に追加してオフラインでも起動できる。`sw.js` は awsquiz と同じ network-first(同一オリジンの GET のみ、api.anthropic.com など外部オリジンは素通し) — キャッシュ対象を変えたら `CACHE_NAME` を必ず上げる)。

Board games (`shogi`, `othello`, `gomoku`, `chess`, `go`, `mahjong`, `diamond-game`) — all self-contained `index.html` apps, all independent from-scratch web implementations kept deliberately separate from `mygames`'s native Swift ボードゲーム tab (same relationship as `shogi/` vs. mygames's native 将棋). Each has 対AI対戦(with a difficulty picker)/2人対戦 modes except `mahjong`, which is inherently 4-player so it's 1人プレイ vs AI×3 instead. Each documents its own known rule simplifications in its `CLAUDE.md` — read that before assuming a rule is fully implemented:
...
- **バージョン表示は必ず上げる。** ランチャーから開く各アプリのトップ画面（最初に出る画面）に `v1.0.0` 形式のバージョンを小さく出している。**そのアプリのファイルを変更したら、同じコミットで必ずバージョンを上げること**（軽微な修正 = パッチ、機能追加 = マイナー）。iPhone のホーム画面に登録した状態では「更新が届いたか」を確認する手段がこれしかないため、省略しない。置き場所は各アプリの `.app-version`（earth のみ固定表示の `#app-version`）:
  - **表示位置は必ず画面の上部**（タイトル/サブタイトルのすぐ下。earth のみ右上に固定表示の `#app-version`）。下端に置くとスクロールしないと見えないため、下には戻さないこと: `earth/index.html`（右上固定）/ `shinkansen/index.html`（ヘッダー内）/ `tarot/index.html`（ヘッダー内）/ `tarot/quiz.html`（イントロのサブタイトル下）/ `kids-learning-app/index.html`（タイトル下）/ `pgquiz/index.html`・`awsquiz/index.html`（`.app-sub` の下）
  ...
  - この更新日時はタップできる（ルート `index.html` 末尾の `<script>`）。押すと `?t=<現在時刻>` を付けて読み込み直し、その状態のときだけ各アプリへのリンクにも同じ `t` を伝播させて、HTTP キャッシュを避けて最新を取りに行く。ホーム画面に登録した web アプリには「引っぱって更新」が無いための代替手段。**通常時にリンクへ `t` を付けてはいけない**（PWA のオフラインキャッシュを外してしまい、kids-learning-app がオフラインで動かなくなる）。
