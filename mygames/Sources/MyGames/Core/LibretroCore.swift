...
final class LibretroCore: ObservableObject {
    ...
    private let emulationQueue = DispatchQueue(label: "com.mygames.emulation", qos: .userInteractive)
    ...
    init() {
        ...
        let base = appSupport.appendingPathComponent("MyGames")
    ...
    private func stateFileURL() -> URL? {
        ...
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return appSupport.appendingPathComponent("MyGames/States/\(name).state")
    }
